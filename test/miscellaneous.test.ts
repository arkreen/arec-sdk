...
describe('miscellaneous', () => {
  it('getLiquidityMinted:0', async () => {
    ...
    expect(() => {
      pair.getLiquidityMinted(
        new TokenAmount(pair.liquidityToken0, '0'),
  ...
  it('getLiquidityMinted:!0', async () => {
    ...
    expect(
      pair
        .getLiquidityMinted(
          new TokenAmount(pair.liquidityToken0, '10000'),
  ...
  it('getLiquidityValue:!feeOn', async () => {
...

    {
      const liquidityValue = pair.getLiquidityValue(
        tokenA,
        new TokenAmount(pair.liquidityToken0, '1000'),
        new TokenAmount(pair.liquidityToken0, '1000'),
        false
      )
    ...
    // 500
    {
      const liquidityValue = pair.getLiquidityValue(
        tokenA,
        new TokenAmount(pair.liquidityToken0, '1000'),
        new TokenAmount(pair.liquidityToken0, '500'),
        false
      )
      expect(liquidityValue.token.equals(tokenA)).toBe(true)
  ...
  it('getLiquidityValue:feeOn', async () => {
    ...
    expect(liquidityValue.token.equals(tokenA)).toBe(true)
    expect(liquidityValue.raw.toString()).toBe('959') // 959: ceiling(1000 - (500 * (1 / 12))) // 917: ceiling(1000 - (500 * (1 / 6)))
  })
